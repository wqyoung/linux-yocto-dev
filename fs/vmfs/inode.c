...
DEFINE_MUTEX(vmfs_mutex);	/* Mutex for VMFS objects. */
...
static struct message_box *mbox;
			   ...
			   int silent)
{
	...
	mnt->file_mode = 0755 | S_IFREG;
	mnt->dir_mode = 0755 | S_IFDIR;
...
out_no_mem:
	...
	return -EINVAL;
out_no_server:
	pr_err("%s: cannot allocate struct vmfs_sb_info\n", __func__);
	return -ENOMEM;
}
...
{
	int err = vmfs_revalidate_inode(path->dentry);

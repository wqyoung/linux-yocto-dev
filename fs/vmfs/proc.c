...
static u64 divmod64(u64 dividend, u32 divisor,
		    u32 *remainder)
{
	u64 quotient = 0;
	u32 pquot, prem = 0;
	...
	/* divide in 4 32x16->32 bit parts. As most ARMs don't have /
	 * anyway we should probably do this bit by bit
	 */
	for (i = 0; i < 4; ++i) {
		u32 part = (prem << 16) | (dividend >> 48);
...
int vmfs_open(struct dentry *dentry, int flags, int wish)
{
	...
	int result;
	enum vfs_open_flags vwish, vaccess;
...
static int vmfs_proc_close(struct vmfs_sb_info *server, s32 handle)
{
	int result = -ENOMEM;
...
int vmfs_proc_create(struct dentry *dentry, u32 mode, s32 *fileid)
{
	struct vmfs_sb_info *server = server_from_dentry(dentry);
	struct vmfs_ws *ws = vmfs_get_ws(server);
	int result;
		      ...
		      struct super_block *sb)
{
	...
	inode = vmfs_iget(sb, fattr);

	if (inode) {
		fattr->f_mtime = current_time(inode);
		       ...
		       struct vmfs_cache_control *ctl)
{
	...
	while (result >= 0) {
		...
		/* mtime/ctime/atime are ms since linux epoch */
		{
			...
			div = (u32)divmod64(mtime, 1000, &mod);
...

		{
			...
			div = (u32)divmod64(ctime, 1000, &mod);
...

		{
			...
			div = (u32)divmod64(atime, 1000, &mod);
		...
		fattr.f_size = fsize;
		fattr.attr = 0;
		if (ftype == (enum vfs_attr)VFS_TYPE_DIR)
			fattr.attr |= ADIR;
...
int vmfs_proc_dskattr(struct dentry *dentry, struct kstatfs *kattr)
{
	...
	struct vmfs_ws *ws = vmfs_get_ws(server);
	u32 attr = VFS_ATTR_DISKSIZE | VFS_ATTR_DISKFREE;
	u8 *attrdata = ws->attr;
	u32 attrlen = 16;
	u64 disksize, diskfree;
	...
	disksize = *(u64 *)attrdata;
	diskfree = *(u64 *)(attrdata + 8);

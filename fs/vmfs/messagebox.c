...
struct message_box {
	struct message_box_regs *dev;	/* virtual base of registers */

	u32 dev_base;	/* physical base of registers */
	u32 dev_irq;	/* irq number of device */
	u32 *buffer;	/* fixed size buffer used for passing data */
	...
	spinlock_t irq_lock;	/* Message box lock. */
#endif

	u32 use_irq;	/* set to true if we're using irq's
			 * rather than polling
			 */
...
{
	struct message_box *mb = (struct message_box *)dev_id;
...
struct message_box *mb_new(phys_addr_t dev_base, u32 dev_irq)
{
	...
	mb = kmalloc(sizeof(*mb), GFP_KERNEL);
	if (!mb)
		return NULL;
...
#ifdef USE_PIO
	mb->buffer = kmalloc(MBOX_BUFFER_SIZE, GFP_KERNEL);
#else
	mb->buffer = (u32 *)((u8 *)mb->dev + MBOX_BUFFER_BASE);
#endif
...
void *mb_receive(struct message_box *mb, u32 *len)
{
	...
	*len = bidx * 4;
	...
	return mb->buffer;
#else
	u32 start = readl(&mb->dev->start);
	u32 end = readl(&mb->dev->end);

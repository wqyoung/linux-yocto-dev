...
static int vmfs_readpage_sync(struct dentry *dentry, struct page *page)
{
	...
	do {
		...
		inode = vmfs_iget(dentry->d_inode->i_sb, &fattr);
		if (inode)
			dentry->d_inode->i_atime = current_time(inode);
		    ...
		    unsigned long pageoffset, unsigned int count)
{
	...
	do {
		...
		tinode = vmfs_iget(inode->i_sb, &fattr);
		if (tinode)
			inode->i_atime = current_time(tinode);
...
static int vmfs_writepage(struct page *page, struct writeback_control *wbc)
{
	...
	unsigned int offset = PAGE_CACHE_SIZE;
	int err;

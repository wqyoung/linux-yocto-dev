 ...
 */
static int vmfs_instantiate(struct dentry *dentry, s32 vhandle,
			    int have_id)
{
	struct inode *inode;
	int error;
 ...
 */
static int
vmfs_make_node(struct inode *dir, struct dentry *dentry, umode_t mode,
	       dev_t dev)
{
	return -EINVAL;
}
...
vmfs_link(struct dentry *dentry, struct inode *dir, struct dentry *new_dentry)
{
	...
	DEBUG1("%s old=%s/%s new=%s/%s\n",
	       __func__, DENTRY_PATH(dentry), DENTRY_PATH(new_dentry));
	vmfs_invalid_dir_cache(dir);
